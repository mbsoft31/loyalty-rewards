...
describe('LoyaltyAccount Model', function () {
    it('creates a new account', function () {
        ...
        expect($account->getCustomerId())->toEqual($customerId)
            ->and($account->getAvailablePoints())->toBeZeroPoints()
            ->and($account->getPendingPoints())->toBeZeroPoints()
            ->and($account->isActive())->toBeTrue()
            ->and($account->getEvents())->toHaveCount(1)
            ->and($account->getEvents()[0])->toBeInstanceOf(AccountCreatedEvent::class);
    });
    ...
    it('earns points successfully', function () {
        ...
        expect($account->getPendingPoints())->toBePoints(150)
            ->and($transaction->getPoints())->toBePoints(150)
            ->and($account->getEvents())->toHaveCount(1)
            ->and($account->getEvents()[0])->toBeInstanceOf(PointsEarnedEvent::class);
    });
    ...
    it('confirms pending points', function () {
        ...
        $account->earnPoints(Factories::points(), Factories::transactionContext());
        ...
        expect($account->getAvailablePoints())->toBePoints(100)
            ->and($account->getPendingPoints())->toBeZeroPoints()
            ->and($account->getLifetimePoints())->toBePoints(100);
    });
    ...
    it('redeems points successfully', function () {
        ...
        expect($account->getAvailablePoints())->toBePoints(125)
            ->and($transaction->getPoints())->toBePoints(75)
            ->and($account->getEvents())->toHaveCount(1)
            ->and($account->getEvents()[0])->toBeInstanceOf(PointsRedeemedEvent::class);
    });
    ...
    it('throws exception when redeeming more points than available', function () {
        ...
        expect(fn () => $account->redeemPoints(Factories::points()))
            ->toThrow(InsufficientPointsException::class, 'Insufficient points');
    });

    it(/**
     * @throws InactiveAccountException
     */ 'adjusts points with positive adjustment', function () {
        $account = Factories::loyaltyAccount(
            availablePoints: Factories::points()
        );

        $transaction = $account->adjustPoints(Factories::points(25), 'Bonus points');

        expect($account->getAvailablePoints())->toBePoints(125)
            ->and($account->getLifetimePoints())->toBePoints(25);
    });
    ...
    it('closes account and clears points', function () {
        $account = Factories::loyaltyAccount(
            ...
            pendingPoints: Factories::points()
        );
    ...
    it('throws exception when operating on inactive account', function () {
        ...
        expect(fn () => $account->earnPoints(Factories::points(), Factories::transactionContext()))
    ...
    it('checks redemption eligibility correctly', function () {
        ...
        expect($accountWithPoints->canRedeemPoints())->toBeTrue()
            ->and($accountWithoutPoints->canRedeemPoints())->toBeFalse()
            ->and($suspendedAccount->canRedeemPoints())->toBeFalse();
    });
});
