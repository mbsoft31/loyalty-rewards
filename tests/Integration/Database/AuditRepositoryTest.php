...
describe('Database Audit Repository Integration', function () {
    ...
    it('finds by customer id entity type', function () {
        $customerId = CustomerId::fromString('customer_abc');
        $rec = AuditRecord::create('loyalty_customer', $customerId->toString(), 'profile_updated');
        $this->auditRepository->store($rec);
        $rows = $this->auditRepository->findByCustomer($customerId);
